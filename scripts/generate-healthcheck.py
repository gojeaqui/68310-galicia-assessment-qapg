...
import sys, yaml, click, csv, re, time
...
4+^| {{set:cellbgcolor:#E6E6E6}} *{config['categories'][category]['text']}*
"""
                    separator = True
...
# Category: {config['categories'][category]['text']}
...
"""
        ...
        for item in items[category]:
            ...
            # Page break between descriptions
            output += "<<<\n\n"
...
"""
    ...
    if ('outcome_evidence' in results and results['outcome_evidence'] != ""):
        output += f"""*Evidencia recolectada de 'must-gather'*
"""
        output += f"""
```
{results['outcome_evidence']}
```
"""

    if ('transcript' in results and results['transcript']['transcript_text'] != ""):
        output += f"""*Transcript of conversation with client {results['transcript']['transcript_date']}*
"""
        output += f"""
```
...
def __generate_summary_row(item: dict, config: dict) -> str:
    "Generate an AsciiDoc Summary Table Row for item file"
    if item['version'] == 'v0':
...
|
{{set:cellbgcolor!}}
{item['check_definition']['subcategory_key']}
...
def __load_config(input_dir: str) -> dict:
    ...
    try:
        with open(config_file) as f:
            ...
            config['skip_summary']: dict = file['skip_summary']
    except KeyError:
    ...
    try:
        with open(config_file) as f:
            file = yaml.safe_load(f)
            config['statuses']: dict = file['statuses']
            config['skip_statuses']: dict = file['skip_statuses']
    ...
    try:
        ...
        print(f"No categories file found at {config_file}", file=sys.stderr)
        sys.exit(1)
...
# Get dict of enabled tags in config.yaml
def __get_enabled_tags(config_file: str) -> dict:
    tags = {}
    with open(config_file) as f:
...
def __ensure_output_dir_exists(output_dir: str):
    if not path.exists(output_dir):
        print(f"Output directory {output_dir} does not exist. Creating it.")
        makedirs(output_dir)

# iterate through the item files, if any of the enabled tags are in the item file tag list then skip - otherwise move to disabled folder
def __filter_items(input_dir: str, output_dir: str, tags: dict):
    move = True
    __ensure_output_dir_exists(output_dir)
    item_files: list[str] = [f
                  for f in sorted(listdir(input_dir))
                  if f.endswith('.item') or f.endswith('.item.yaml')]
    
    for item_file in item_files:
        with open(input_dir + item_file) as f:
            item_tags = yaml.safe_load(f)['check_definition']['tags']
            for tag in item_tags:
                ...
                if tags[tag] == True:
                    ...
                    break
            if move:
                print(input_dir + item_file)
                rename(input_dir + item_file, output_dir + item_file)
            move = True

def __unfilter(input_dir: str, output_dir: str):
    item_files: list[str] = [f
                  for f in sorted(listdir(input_dir))
                  if f.endswith('.item') or f.endswith('.item.yaml')]
...
#click command to unfilter
@cli.command()
@click.option('--input-dir', default="./content/healthcheck-items/disabled/", show_default=True, type=click.Path(exists=True, file_okay=False))
@click.option('--output-dir', default="./content/healthcheck-items/", show_default=True, type=click.Path(file_okay=False))
def unfilter(input_dir: str, output_dir: str):
    "Unfilter items"

    __unfilter(input_dir, output_dir)

#click command to filter items based on tags
@cli.command()
@click.option('--input-dir', default="./content/healthcheck-items/", show_default=True, type=click.Path(exists=True, file_okay=False))
@click.option('--output-dir', default="./content/healthcheck-items/disabled/", show_default=True, type=click.Path(file_okay=False))
def filter(input_dir: str, output_dir: str):
    "Filter items based on tags"

    tags = __get_enabled_tags(input_dir + "config.yaml")
    __filter_items(input_dir, output_dir, tags)
        ...
        schema = {
            ...
            "customer_environment_findings": {
                ...
                "schema": {
                    ...
                    "outcome_evidence": {"type": "string", "required": False},
                    "transcript": {
                        "type": "dict",
                        "required": False,
                        "schema": {
                            "transcript_date": {"type": "string", "required": True},
